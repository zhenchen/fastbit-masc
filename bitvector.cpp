...
void ibis::bitvector::compress_masc()
{
	compress();
	if (m_vec.size() < 2 || m_vec.incore() == false) // there is nothing to do
	return;
	// in fact should take cases that masc longer than wah into consideration. Not considered here yet.
 struct xrun {
	bool   isFill;
	int    fillBit;
	bool	mascType;
	int		mascCnt;
	int		mascCntPre;
	bool	isCarrier; //0 means no carrier, 1 means carrier
	word_t nWords;
	...
	array_t<word_t> mascArray(m_vec.size() + 1,0);

	bitvector * masc_vec = new bitvector(mascArray);
	xrun currentWah, currentMasc;
	currentWah.it = m_vec.begin();
	currentMasc.it = masc_vec->m_vec.begin();
	
	currentMasc.decode();
	for(currentWah.it; currentWah.it < m_vec.end(); currentWah.it++)
	{
		currentWah.decode();
		if(currentWah.isFill) //fill
		{
			if(!currentWah.fillBit) //0-fill
			{
				if(!currentMasc.mascType)
				{
					if(!currentMasc.isCarrier) // no carrier
					{
						currentMasc.mascCnt += 31;
					}
					else //has carrier
					{
						*currentMasc.it = 0x40000000;
						*currentMasc.it += currentMasc.mascCntPre << 5;
						*currentMasc.it += currentMasc.mascCnt;
						currentMasc.it++;
						currentMasc.decode();
						currentMasc.mascCnt += 31;
					}
			...
			{
				if(currentMasc.mascType)//1-fill
				{
					currentMasc.mascCnt+=31;
				}
				else if(currentMasc.isCarrier)//carried
				{
					*currentMasc.it = currentMasc.mascCntPre;
					currentMasc.it++;
					int tmpCnt = currentMasc.mascCnt;
					currentMasc.decode();
					currentMasc.mascType = true;
					currentMasc.mascCnt = 31 + tmpCnt;
				}
				else //0-fill
				{
					*currentMasc.it = currentMasc.mascCnt;
		...
		{
			int tmpPos;
			for(tmpPos = 30; tmpPos>=0; tmpPos--) //varies based on big-end or small-end
			{
				if(currentWah.it[tmpPos] == 0)
				{
					if(currentMasc.mascType)//1-fill
					{*currentMasc.it = 0x80000000;
					*currentMasc.it += currentMasc.mascCnt;
					...
					currentMasc.decode();
					currentMasc.mascCnt += 31;}
					else if(currentMasc.isCarrier) // 0-fill, carry
					{
							*currentMasc.it = 0x40000000;
							*currentMasc.it += currentMasc.mascCnt;
							*currentMasc.it += (currentMasc.mascCntPre << 5);
				...
				else // happens 1
				{
					...
					{
						currentMasc.mascCnt ++;
						if(currentMasc.mascCnt >= 31)//split
						{
							*currentMasc.it = currentMasc.mascCntPre;
							currentMasc.it++;
							int tmpCnt = currentMasc.mascCnt;
							currentMasc.decode();
							currentMasc.mascType = true;
							currentMasc.mascCnt = tmpCnt;
						}
						else ; //not overflow. do nothing.
					}
					else// 0-fill, not carry
					{
						*currentMasc.it = currentMasc.mascCnt;
	...
	masc_vec->m_vec.erase(currentMasc.it, masc_vec->m_vec.end());
	m_vec.swap(masc_vec->m_vec);//exanchge the two arrays.
	free(masc_vec);
}
...
void ibis::bitvector::decompress_masc()
{
	struct xrun {
	bool fillType;
	bool isCarried;	
	word_t bitCounter;
	word_t carrierCounter;
	array_t<word_t>::iterator it;
	
	xrun() : fillType(false), isCarried(false), bitCounter(0), carrierCounter(0),it(0) {};
	void decode() {
		fillType = (*it > ALLONES);
	    isCarried = (*it >= 0xc0000000);
		if(fillType){ //1-fill
			bitCounter = (*it & 0x7fffffff);
		}
		else{ //0-fill
			if(isCarried){ // Carried
				carrierCounter = (*it & 0x0000001f);
				bitCounter = ((*it & 0x3fffffe0) >> 5);
			}
			else{ // Not Carried
				carrierCounter = (*it & 0x3fffffff);
			}
		}
	   }
	};
	...
	for(current.it;current.it<m_vec.end();current.it++){
		current.decode();
		if(current.isCarried){
			totalBits += current.carrierCounter;
		}
		totalBits += current.bitCounter;
	}
	
	int totalWords = totalBits/31;
	int currentBits = 0;
	int bitPos = 0;
	int wordPos = 0;
	int tmp;
	array_t<word_t> tmp_array(totalWords+1,0);
	bitvector * tmp_vec = new bitvector(tmp_array);

	current.it = m_vec.begin();
	for(current.it;current.it<m_vec.end();current.it++){
		current.decode();
		if(current.fillType){ //1-fill
			currentBits = current.bitCounter;
			for(tmp=0;tmp<currentBits;tmp++)
			{
				if(bitPos==31){//this one is full, next word;
					wordPos++;
					bitPos = 0;
					tmp_vec->m_vec[wordPos] = 1;
				}
				else{ //not full
					bitPos++;
					tmp_vec->m_vec[wordPos] = (tmp_vec->m_vec[wordPos] << 1);
					tmp_vec->m_vec[wordPos] ++;		
				}
			}
		}
		else if(current.carrierCounter){ //0-fill, carried
			currentBits = current.bitCounter;
			for(tmp=0;tmp<currentBits;tmp++)
			{
				if(bitPos==31){//this one is full, next word;
					wordPos++;
					bitPos = 0;
					tmp_vec->m_vec[wordPos] = 0;
				}
				else{ //not full
				bitPos++;}
			}
			currentBits = current.carrierCounter;
			for(tmp=0;tmp<currentBits;tmp++)
			{
				if(bitPos==31){//this one is full, next word;
					wordPos++;
